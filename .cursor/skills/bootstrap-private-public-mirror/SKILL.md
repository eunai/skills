...
- Detect start-of-run overrides and print them in Phase 1: `force fresh`, `force convert`, `make AGENTS.md public`, `also write bash publisher`, `include code`.
...
- Phase 3: create or merge `README.public.md` and `CHANGELOG.public.md` using the filter rules and one-line HTML banner in `REFERENCE.md`; optionally create `.gitignore.public`. In CONVERT, show a mandatory unified diff per file.
- Phase 4: add the private `README.md` Publishing section if missing, adapted to local style. Do not modify private `CHANGELOG.md`.
- Phase 5: build one in-memory source list for whitelist paths, path renames, and forbidden patterns, then render both `scripts/publish.ps1` and `docs/internal/publish_whitelist_tree.md`. On rejection, use the targeted Revise / Abort / Skip-degraded prompt.
...
- Phase 8: offer dry-run validation after tool preflight. On Y, run `pwsh -File scripts/publish.ps1`, capture output, surface the required result lines, and write `docs/internal/bootstrap-dry-run-YYYY-MM-DD.log` using the pack same-day numbering rule. On N, print later-run and publish commands.

## Publisher Requirements

- `scripts/publish.ps1` uses `#Requires -Version 5.1`, strict mode, `-Push`, `-PushTags`, `-SkipDocCheck`, detected `$PublicRemote`, and detected branch. Never leave `<REPLACE_ME>`.
